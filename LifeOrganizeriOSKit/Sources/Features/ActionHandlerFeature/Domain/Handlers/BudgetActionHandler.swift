...
public actor BudgetActionHandler: ActionHandlerProtocol {
    public typealias ActionDataType = BudgetAction
    ...
    public func handle(_ action: BudgetAction) async throws -> ActionHandlerResult {
        ...
        if let category = action.category, category.trimmingCharacters(in: .whitespaces).isEmpty {
            throw AppError.actionHandler(.invalidAction("Category cannot be empty"))
        }
        ...
        return ActionHandlerResult(
            success: true,
            message: "Budget action validated successfully. Amount: \(action.amount), Type: \(action.transactionType)"
        )
    }
}
